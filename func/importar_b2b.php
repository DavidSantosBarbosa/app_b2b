...
$limpar_tabela = $pdo->query("TRUNCATE TABLE appb2b_base_eventos_b2b");
...
if (strtolower(end($verificar_extencao)) == 'csv') {
    ...
    while($linhas = fgetcsv($dados, 1000, ';')){
        ...
        $cmd = $pdo->query("insert into appb2b_base_eventos_b2b values('$linhas[0]','$linhas[1]','$linhas[2]','$linhas[3]','$linhas[4]','$linhas[5]','$linhas[6]','$linhas[7]',now())");
        /* $cmd->execute(array(
            ':bd' => $linhas[0],
